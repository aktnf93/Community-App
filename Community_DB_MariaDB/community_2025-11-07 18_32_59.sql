...
CREATE TABLE IF NOT EXISTS `tb_employee_privileges` (
  ...
  `privilege_id` int(10) unsigned NOT NULL COMMENT '권한 ID',
  `is_active` enum('Y','N') NOT NULL DEFAULT 'Y' COMMENT '권한 허용 여부',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성시각',
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '레코드 수정시각',
  PRIMARY KEY (`id`),
  KEY `FK_tb_employee_privileges_tb_employees` (`employee_id`),
  KEY `FK_tb_employee_privileges_tb_system_privileges` (`privilege_id`) USING BTREE,
  ...
  CONSTRAINT `FK_tb_employee_privileges_tb_system_privileges` FOREIGN KEY (`privilege_id`) REFERENCES `tb_system_privileges` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='프로젝트 리스트';
...
CREATE TABLE IF NOT EXISTS `tb_system_logs` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성시각',
  PRIMARY KEY (`id`),
  KEY `FK_tb_system_logs_tb_system_log_list` (`log_id`),
  CONSTRAINT `FK_tb_system_logs_tb_system_log_list` FOREIGN KEY (`log_id`) REFERENCES `tb_system_log_list` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
