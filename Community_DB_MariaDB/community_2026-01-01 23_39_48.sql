...
-- 테이블 community.tb_approval_document 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_approval_document` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '레코드 id',
  `type` enum('휴가','지출') NOT NULL COMMENT '문서 종류 Vacation, Expense',
  `title` varchar(50) NOT NULL COMMENT '문서 제목',
  `employee_id` int(10) unsigned NOT NULL COMMENT '문서 작성자',
  `status` enum('대기','진행','승인','거부') NOT NULL COMMENT '문서 상태',
  ...
  PRIMARY KEY (`id`),
  KEY `FK_tb_approval_document_tb_employees` (`employee_id`),
  CONSTRAINT `FK_tb_approval_document_tb_employees` FOREIGN KEY (`employee_id`) REFERENCES `tb_employees` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=8 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='결재 문서 공통';

-- 테이블 데이터 community.tb_approval_document:~5 rows (대략적) 내보내기
INSERT INTO `tb_approval_document` (`id`, `type`, `title`, `employee_id`, `status`, `created_at`, `updated_at`) VALUES
	(2, '휴가', '휴가 신청합니다', 13, '대기', '2025-12-20 15:33:44', '2025-12-21 06:28:44'),
	(3, '휴가', '휴가 승인 진행 중', 13, '진행', '2025-12-21 06:23:36', '2025-12-21 06:29:10'),
	(4, '휴가', '휴가 승인 !', 13, '승인', '2025-12-21 06:23:42', '2025-12-21 06:29:15'),
	(5, '휴가', '휴가를 거부합니다', 13, '거부', '2025-12-21 06:23:42', '2025-12-21 06:29:20'),
	(6, '휴가', '내용 없음', 13, '대기', '2025-12-21 06:23:42', '2025-12-21 06:29:25'),
	(7, '휴가', '내용 없음 123', 13, '대기', '2025-12-21 06:23:42', '2025-12-21 06:29:29');

-- 테이블 community.tb_approval_expense 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_approval_expense` (
  `approval_id` int(10) unsigned NOT NULL COMMENT '결재 문서 id',
  `amount` int(10) NOT NULL COMMENT '사용 금액',
  `purpose` varchar(255) NOT NULL COMMENT '사용 목적',
  `payment` enum('현금','개인카드','법인카드','어음','계좌이체','외상') NOT NULL COMMENT '결재 수단',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성 시각',
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '레코드 수정 시각',
  KEY `FK_tb_approval_expense_tb_approval_document` (`approval_id`),
  CONSTRAINT `FK_tb_approval_expense_tb_approval_document` FOREIGN KEY (`approval_id`) REFERENCES `tb_approval_document` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='지출 결의서';

-- 테이블 데이터 community.tb_approval_expense:~0 rows (대략적) 내보내기

-- 테이블 community.tb_approval_line 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_approval_line` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '레코드  id',
  `approval_id` int(10) unsigned NOT NULL COMMENT '결재 문서 id',
  `employee_id` int(10) unsigned NOT NULL COMMENT '결재자 id',
  `step_order` int(11) unsigned NOT NULL COMMENT '결재 순서',
  `status` enum('대기','승인','반려') NOT NULL COMMENT '결재 상태',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성 시각',
  PRIMARY KEY (`id`),
  KEY `FK_tb_approval_line_tb_approval_document` (`approval_id`),
  KEY `FK_tb_approval_line_tb_employees` (`employee_id`),
  CONSTRAINT `FK_tb_approval_line_tb_approval_document` FOREIGN KEY (`approval_id`) REFERENCES `tb_approval_document` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_tb_approval_line_tb_employees` FOREIGN KEY (`employee_id`) REFERENCES `tb_employees` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='결재 라인';

-- 테이블 데이터 community.tb_approval_line:~0 rows (대략적) 내보내기

-- 테이블 community.tb_approval_vacation 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_approval_vacation` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '레코드 id',
  `approval_id` int(10) unsigned NOT NULL COMMENT '결재 문서 id',
  `type` enum('연차','월차','반차','병가','공가','무급','특별') NOT NULL COMMENT '휴가 종류',
  `start_at` timestamp NOT NULL COMMENT '휴가 시작일',
  `end_at` timestamp NOT NULL COMMENT '휴가 종료일',
  `description` varchar(255) NOT NULL COMMENT '휴가 사유',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성 시각',
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '레코드 수정 시각',
  PRIMARY KEY (`id`),
  UNIQUE KEY `approval_id` (`approval_id`),
  KEY `FK_tb_approval_vacation_tb_approval_document` (`approval_id`),
  CONSTRAINT `FK_tb_approval_vacation_tb_approval_document` FOREIGN KEY (`approval_id`) REFERENCES `tb_approval_document` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='휴가 신청서';

-- 테이블 데이터 community.tb_approval_vacation:~0 rows (대략적) 내보내기
...
) ENGINE=InnoDB AUTO_INCREMENT=70 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='채팅방 별 멤버';

-- 테이블 데이터 community.tb_chat_members:~1 rows (대략적) 내보내기
...
) ENGINE=InnoDB AUTO_INCREMENT=20 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='채팅방 별 메시지';

-- 테이블 데이터 community.tb_chat_messages:~15 rows (대략적) 내보내기
INSERT INTO `tb_chat_messages` (`id`, `chat_room_id`, `employee_id`, `message`, `created_at`, `deleted_at`) VALUES
	...
	(11, 4, 13, '', '2025-12-31 17:02:08', NULL),
	(12, 4, 13, '', '2025-12-31 17:03:18', NULL),
	(13, 4, 13, '', '2025-12-31 17:05:35', NULL),
	(14, 4, 13, '', '2025-12-31 17:06:25', NULL),
	(15, 4, 13, '323232', '2025-12-31 17:06:58', NULL),
	(16, 4, 13, '323232', '2025-12-31 17:07:17', NULL),
	(17, 4, 13, 'ffsd', '2025-12-31 17:08:14', NULL),
	(18, 4, 13, 'ffsdvsdvsd', '2025-12-31 17:08:21', NULL),
	(19, 3, 13, '111', '2026-01-01 14:13:32', NULL);
...
INSERT INTO `tb_employees` (`id`, `name`, `description`, `employee_code`, `employee_type`, `gender`, `birth_date`, `email`, `phone`, `address`, `image_path`, `status`, `joined_at`, `resigned_at`, `resigned_desc`, `team_id`, `rank_id`, `position_id`, `role_id`, `privilege_id`, `login_id`, `login_pw`, `is_active`, `created_at`, `updated_at`, `deleted_at`) VALUES
	(13, '시스템 관리자', '초창기 직원', 'A0001', '정규직', '남', '2025-11-19', 'jameson@123.com', '000-1234-5678', '서울특별시 강남구 서초길 럭셔리브랜드아파트 1232-567, 1999호', '/uploads\\A0006.png', '재직', '2025-11-19 08:30:50', '2025-12-08 13:29:28', '자진퇴사', 31, 14, 10, 12, 2, '123', '123', 'Y', '2025-11-11 05:15:04', '2025-12-21 14:25:09', NULL),
	(15, '김민수', '', 'A0002', '정규직', '남', '2025-11-19', 'jameson@123.com', '110-12**-56**', '00시 00구 00군 0000-123', '/uploads\\A0006.png', '재직', '2025-11-19 08:30:50', NULL, NULL, 31, 13, 7, 13, 2, '김민수', '123', 'Y', '2025-11-11 05:15:04', '2025-12-21 14:25:08', NULL),
	(16, '김영호', '', 'A0003', '정규직', '남', '2025-11-19', 'jameson@123.com', '000-1234-5678', '00시 00구 00군 0000-123', '/uploads\\A0006.png', '휴가', '2025-11-19 08:30:50', NULL, NULL, 31, 13, 7, 15, 2, '김영호', '123', 'Y', '2025-11-11 05:15:04', '2025-12-21 14:25:07', NULL),
	(17, '배선호', '', 'A0004', '인턴', '남', '2025-11-19', '배선호@email.com', '000-1234-5678', '', '/uploads\\A0006.png', '재직', '2025-11-19 08:30:50', NULL, NULL, 31, 11, 7, 13, 2, NULL, NULL, 'N', '2025-11-11 05:15:04', '2025-12-21 14:25:07', NULL),
	(18, '장현석', '직원설명4', 'A0005', NULL, '남', '2025-11-19', 'jameson@123.com', '000-1234-5678', '00시 00구 00군 0000-123', '/uploads\\A0006.png', '퇴직', '2025-11-19 08:30:50', NULL, NULL, NULL, 13, 7, 20, NULL, NULL, NULL, 'N', '2025-11-11 05:15:04', '2025-12-21 14:25:02', NULL),
	(19, '한민우', '직원설명5', 'A0006', NULL, '여', '2025-11-19', 'jameson@123.com', '000-1234-5678', '00시 00구 00군 0000-123', '/uploads\\A0006.png', '재직', '2025-11-19 08:30:50', NULL, NULL, NULL, 13, 7, 20, NULL, NULL, NULL, 'N', '2025-11-11 05:15:04', '2025-12-21 14:25:04', NULL),
	(20, '고선재', '직원설명6', 'A0007', NULL, '여', '2025-11-19', 'jameson@123.com', '055-**12-0099', '서울시 00구 00구 123-567', '/uploads\\A0006.png', '재직', '2025-11-19 08:30:50', NULL, NULL, NULL, 11, 7, 20, NULL, NULL, NULL, 'N', '2025-11-11 05:15:04', '2025-12-21 14:25:05', NULL),
	(21, '박진철', '직원설명7', 'A0008', NULL, '여', '2025-11-19', 'jameson@123.com', '000-1234-5678', '00시 서울구 00-1234', '/uploads\\A0006.png', '퇴직', '2025-11-19 08:30:50', NULL, NULL, 7, 13, 7, 20, NULL, NULL, NULL, 'N', '2025-11-11 05:15:04', '2025-12-21 14:25:06', NULL);
...
) ENGINE=InnoDB AUTO_INCREMENT=35 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='직원 별 근태이력';

-- 테이블 데이터 community.tb_employee_attendance:~24 rows (대략적) 내보내기
INSERT INTO `tb_employee_attendance` (`id`, `employee_id`, `start_work_at`, `end_work_at`, `description`, `created_at`, `updated_at`) VALUES
	...
	(26, 13, '2025-12-11 00:51:02', '2025-12-11 08:43:52', NULL, '2025-12-11 00:51:02', '2025-12-11 08:43:52'),
	(27, 13, '2025-12-12 01:31:20', NULL, NULL, '2025-12-12 01:31:20', '2025-12-12 01:31:20'),
	(28, 13, '2025-12-15 10:05:11', NULL, NULL, '2025-12-15 10:05:11', '2025-12-15 10:05:11'),
	(29, 13, '2025-12-18 02:00:52', '2025-12-18 08:32:55', NULL, '2025-12-18 02:00:52', '2025-12-18 08:32:55'),
	(30, 13, '2025-12-20 15:23:27', '2025-12-21 14:23:40', NULL, '2025-12-20 15:23:27', '2025-12-21 14:23:40'),
	(31, 13, '2025-12-22 04:53:11', NULL, NULL, '2025-12-22 04:53:11', '2025-12-22 04:53:11'),
	(32, 13, '2025-12-23 01:31:35', '2025-12-23 08:42:59', NULL, '2025-12-23 01:31:35', '2025-12-23 08:42:59'),
	(33, 13, '2025-12-27 13:33:50', NULL, NULL, '2025-12-27 13:33:50', '2025-12-27 13:33:50'),
	(34, 13, '2026-01-01 14:13:26', NULL, NULL, '2026-01-01 14:13:26', '2026-01-01 14:13:26');
...
-- 테이블 데이터 community.tb_organization_companies:~0 rows (대략적) 내보내기
...
CREATE TABLE IF NOT EXISTS `tb_posts` (
  ...
  `comment_count` int(10) unsigned NOT NULL DEFAULT 0 COMMENT '댓글 수',
  `post_level` int(10) unsigned DEFAULT NULL COMMENT '게시물 수준 (0=시스템전체, 1=전체, 2=상위카테고리, 3=하위카테고리)',
...
) ENGINE=InnoDB AUTO_INCREMENT=78 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='게시글 리스트';

-- 테이블 데이터 community.tb_posts:~11 rows (대략적) 내보내기
INSERT INTO `tb_posts` (`id`, `post_category_id`, `employee_id`, `title`, `content`, `view_count`, `comment_count`, `post_level`, `comment_at`, `created_at`, `updated_at`, `deleted_at`) VALUES
	(59, 22, 13, 'CC 코페레이션 변경 계약 건', '# CC 코페레이션 변경 계약 건\r\n\r\n- 내용 ...', 3, 0, NULL, NULL, '2025-11-13 06:48:11', '2025-12-12 06:55:44', NULL),
	(60, 8, 13, '자유게시판1 첫 글입니다.', '안녕하세요', 0, 0, NULL, NULL, '2025-11-13 06:48:57', '2025-11-13 06:48:57', NULL),
	(61, 10, 13, '회의 일정 공지', '# 회의 일정 공지\r\n\r\n- 내용 ...', 2, 0, NULL, NULL, '2025-11-13 06:50:32', '2025-12-12 06:32:12', NULL),
	(62, 9, 13, '업무 협조 규칙', '# 업무 협조 규칙\r\n\r\n- 내용 ...', 2, 0, NULL, NULL, '2025-11-13 06:51:54', '2025-12-12 06:32:34', NULL),
	(63, 1, 13, '동호회 지원금 신청 공지', '# 동호회 지원금 신청 공지\r\n\r\n- 많은 관심 부탁드립니다.', 5, 0, 1, NULL, '2025-11-13 06:53:58', '2025-12-21 06:31:12', NULL),
	(64, 1, 13, '신규 입사자 환영 공지', '# 신규 입사자 환영 공지\r\n\r\n- 많은 관심 부탁드립니다.', 3, 0, 1, NULL, '2025-11-13 06:55:35', '2025-12-12 06:54:54', NULL),
	(65, 1, 13, '그룹웨어 신규 기능 업데이트', '# 그룹웨어 신규 기능 업데이트\r\n\r\n- 많은 관심 부탁드립니다.', 6, 0, 1, NULL, '2025-11-13 07:03:59', '2025-12-12 06:55:46', NULL),
	(66, 1, 13, '송년회 일정 및 장소 안내', '# 송년회 일정 및 장소 안내\r\n\r\n- 많은 관심 부탁드립니다.', 5, 0, 1, NULL, '2025-11-19 06:14:46', '2025-12-23 08:12:51', NULL),
	(67, 1, 13, '신규 사내 규정 적용 일정', '# 신규 사내 규정 적용 일정\r\n\r\n- 많은 참석 바랍니다.', 56, 7, 1, '2025-12-21 06:31:40', '2025-11-19 06:16:12', '2025-12-21 14:26:38', NULL),
	(76, 8, 13, '1', '1', 1, 0, NULL, NULL, '2025-12-12 08:09:31', '2025-12-12 08:23:45', NULL),
	(77, 8, 13, '새 게시물 테스트', '내용 123', 2, 0, NULL, NULL, '2025-12-15 10:05:58', '2025-12-15 10:06:25', NULL);

-- 테이블 community.tb_post_category 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_post_category` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '레코드 id',
  `parent_id` int(10) unsigned DEFAULT NULL COMMENT '상위 카테고리 id',
  `name` varchar(255) NOT NULL COMMENT '카테고리 이름',
  `description` varchar(255) DEFAULT NULL COMMENT '카테고리 설명',
  `post_count` int(10) unsigned NOT NULL DEFAULT 0 COMMENT '게시글 수',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성 시각',
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '레코드 수정 시각',
  PRIMARY KEY (`id`),
  UNIQUE KEY `name` (`name`),
...
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='게시글 카테고리 정보';

-- 테이블 데이터 community.tb_post_category:~13 rows (대략적) 내보내기
INSERT INTO `tb_post_category` (`id`, `parent_id`, `name`, `description`, `post_count`, `created_at`, `updated_at`) VALUES
	(1, NULL, '공지사항', NULL, 0, '2025-11-11 10:16:30', '2025-12-05 05:24:38'),
	(7, NULL, '일반 게시판', NULL, 0, '2025-11-11 10:12:29', '2025-12-12 06:10:09'),
	(8, 7, '자유 게시판', NULL, 2, '2025-11-11 10:12:41', '2025-12-15 10:05:58'),
	(9, 7, '실무자 게시판', NULL, 0, '2025-11-11 10:12:49', '2025-12-12 06:10:31'),
	(10, 7, '부서장 게시판', NULL, 0, '2025-11-11 10:12:56', '2025-12-12 06:10:37'),
	(11, NULL, '부서 게시판', NULL, 0, '2025-11-11 10:13:11', '2025-11-11 11:20:21'),
	(12, 11, '마케팅 & 기획', NULL, 0, '2025-11-11 10:13:28', '2025-12-11 05:13:17'),
	(15, 11, '개발', NULL, 0, '2025-11-11 10:15:20', '2025-11-11 10:15:20'),
	(16, 11, '영업 & 총무', NULL, 0, '2025-11-11 10:15:29', '2025-12-11 05:13:29'),
	(18, NULL, '고객의 소리', NULL, 0, '2025-11-11 10:16:24', '2025-11-11 10:16:24'),
	(20, 18, 'LL 청과물', NULL, 0, '2025-11-11 10:16:49', '2025-12-12 06:11:11'),
	(21, 18, 'YY 엔터테이먼트', NULL, 0, '2025-11-11 10:17:31', '2025-12-12 06:11:44'),
	(22, 18, '(주) CC 코퍼레이션', NULL, 0, '2025-11-11 10:17:48', '2025-12-12 06:12:45');
...
CREATE TABLE IF NOT EXISTS `tb_post_comments` (
  ...
  CONSTRAINT `FK_tb_post_comments_tb_posts` FOREIGN KEY (`post_id`) REFERENCES `tb_posts` (`id`) ON DELETE CASCADE ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='게시글 별 댓글';

-- 테이블 데이터 community.tb_post_comments:~7 rows (대략적) 내보내기
INSERT INTO `tb_post_comments` (`id`, `post_id`, `employee_id`, `content`, `created_at`, `updated_at`, `deleted_at`) VALUES
	(2, 67, 13, '댓글1', '2025-12-11 04:08:55', '2025-12-11 04:08:55', NULL),
	(3, 67, 13, '32', '2025-12-11 08:16:53', '2025-12-11 08:16:53', NULL),
	(4, 67, 13, '댓글2', '2025-12-11 08:33:09', '2025-12-11 08:33:09', NULL),
	(5, 67, 13, 'ㅇㅁㄴㅇㅁㄴ', '2025-12-11 08:33:15', '2025-12-11 08:33:15', NULL),
	(6, 67, 13, 'ㅊㅌㅊ', '2025-12-11 08:33:17', '2025-12-11 08:33:17', NULL),
	(7, 67, 13, 'ewe', '2025-12-11 08:35:32', '2025-12-11 08:35:32', NULL),
	(9, 67, 13, '참석합니다.', '2025-12-21 06:31:40', '2025-12-21 06:31:40', NULL);
...
) ENGINE=InnoDB AUTO_INCREMENT=62 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='프로젝트 리스트';

-- 테이블 데이터 community.tb_projects:~7 rows (대략적) 내보내기
INSERT INTO `tb_projects` (`id`, `customer_id`, `name`, `description`, `progress`, `start_date`, `end_date`, `status`, `created_at`, `updated_at`, `deleted_at`) VALUES
	...
	(60, 1, '새 프로젝트', '설명', 60, '2025-09-30 15:00:00', '2025-11-29 15:00:00', '진행', '2025-12-15 11:09:59', '2025-12-15 11:09:59', NULL);
...
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='프로젝트 별 멤버';

-- 테이블 데이터 community.tb_project_members:~4 rows (대략적) 내보내기
INSERT INTO `tb_project_members` (`id`, `project_id`, `employee_id`, `created_at`) VALUES
	(14, 60, 13, '2025-12-15 11:09:59'),
	(15, 60, 15, '2025-12-15 11:09:59'),
	(16, 60, 16, '2025-12-15 11:09:59'),
	(17, 60, 17, '2025-12-15 11:09:59');
...
) ENGINE=InnoDB AUTO_INCREMENT=44 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='프로젝트 별 작업';

-- 테이블 데이터 community.tb_project_tasks:~7 rows (대략적) 내보내기
INSERT INTO `tb_project_tasks` (`id`, `project_id`, `task_no`, `name`, `description`, `progress`, `start_date`, `end_date`, `status`, `created_at`, `updated_at`) VALUES
	...
	(38, 60, 0, '새 작업1', NULL, 0, '2024-12-31 15:00:00', '2025-02-27 15:00:00', '대기', '2025-12-15 11:09:59', '2025-12-15 11:09:59'),
	(39, 60, 0, '새 작업2', NULL, 6, '2025-02-28 15:00:00', '2025-07-30 15:00:00', '진행', '2025-12-15 11:09:59', '2025-12-15 11:09:59'),
	(40, 60, 0, '새 작업3', NULL, 100, '2025-03-26 15:00:00', '2025-05-29 15:00:00', '완료', '2025-12-15 11:09:59', '2025-12-15 11:09:59'),
	(41, 60, 0, '새 작업4', NULL, 20, '2025-12-14 15:00:00', '2025-12-18 15:00:00', '취소', '2025-12-15 11:09:59', '2025-12-15 11:09:59'),
	(42, 60, 0, '새 작업5', NULL, 30, '2025-05-04 15:00:00', '2025-12-14 15:00:00', '보류', '2025-12-15 11:09:59', '2025-12-15 11:09:59'),
	(43, 60, 0, '새 작업6', NULL, 13, '2025-01-15 15:00:00', '2025-04-15 15:00:00', '대기', '2025-12-15 11:09:59', '2025-12-15 11:09:59');
...
) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='프로젝트 별 작업 멤버';

-- 테이블 데이터 community.tb_project_task_members:~6 rows (대략적) 내보내기
INSERT INTO `tb_project_task_members` (`id`, `project_task_id`, `employee_id`, `created_at`) VALUES
	(11, 38, 13, '2025-12-15 11:09:59'),
	(12, 39, 15, '2025-12-15 11:09:59'),
	(13, 40, 16, '2025-12-15 11:09:59'),
	(14, 41, 17, '2025-12-15 11:09:59'),
	(15, 42, 13, '2025-12-15 11:09:59'),
	(16, 43, 13, '2025-12-15 11:09:59');

-- 테이블 community.tb_schedule 구조 내보내기
CREATE TABLE IF NOT EXISTS `tb_schedule` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '레코드 id',
  `employee_id` int(10) unsigned NOT NULL COMMENT '직원 id',
  `title` varchar(50) NOT NULL DEFAULT '' COMMENT '일정 제목',
  `start_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '일정 시작',
  `end_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '일정 종료',
  `color_code` int(11) NOT NULL DEFAULT 0 COMMENT '일정 색상',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성 시각',
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '레코드 수정 시각',
  PRIMARY KEY (`id`),
  KEY `FK_tb_schedule_tb_employees` (`employee_id`),
  CONSTRAINT `FK_tb_schedule_tb_employees` FOREIGN KEY (`employee_id`) REFERENCES `tb_employees` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=69 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 테이블 데이터 community.tb_schedule:~23 rows (대략적) 내보내기
INSERT INTO `tb_schedule` (`id`, `employee_id`, `title`, `start_at`, `end_at`, `color_code`, `created_at`, `updated_at`) VALUES
	(9, 13, '연말회의', '2025-12-07 15:00:00', '2025-12-07 15:00:00', 2, '2025-12-22 06:15:32', '2025-12-22 12:11:18'),
	(10, 13, '연말정리', '2025-12-08 15:00:00', '2025-12-11 15:00:00', 3, '2025-12-22 06:20:59', '2025-12-22 12:10:21'),
	(15, 13, '휴가', '2025-12-18 15:00:00', '2025-12-18 15:00:00', 7, '2025-12-22 06:53:10', '2025-12-22 12:12:00'),
	(16, 13, '장보는 날', '2025-12-19 15:00:00', '2025-12-19 15:00:00', 7, '2025-12-22 06:53:24', '2025-12-22 12:12:13'),
	(17, 13, '청소', '2025-12-20 15:00:00', '2025-12-20 15:00:00', 7, '2025-12-22 06:53:39', '2025-12-22 12:12:19'),
	(18, 13, '행사 준비 및 진행', '2025-12-01 15:00:00', '2025-12-02 15:00:00', 2, '2025-12-22 06:53:52', '2025-12-22 12:11:31'),
	(19, 13, '26년 계획표 정리', '2025-12-01 15:00:00', '2025-12-01 15:00:00', 1, '2025-12-22 06:54:07', '2025-12-22 12:11:36'),
	(20, 13, '25년 계획표 정리', '2025-12-01 15:00:00', '2025-12-01 15:00:00', 1, '2025-12-22 06:54:32', '2025-12-22 12:11:39'),
	(21, 13, '외부 미팅 ( 00 회사 협업 )', '2025-12-08 15:00:00', '2025-12-08 15:00:00', 4, '2025-12-22 06:54:56', '2025-12-22 12:11:45'),
	(22, 13, '17:00~ 부서 회식', '2025-12-28 15:00:00', '2025-12-28 15:00:00', 2, '2025-12-22 06:55:28', '2025-12-22 12:11:27'),
	(23, 13, '인수인계', '2025-12-17 15:00:00', '2025-12-17 15:00:00', 3, '2025-12-22 06:55:44', '2025-12-22 12:11:52'),
	(24, 13, '휴가 준비', '2025-12-17 15:00:00', '2025-12-17 15:00:00', 7, '2025-12-22 06:56:00', '2025-12-22 12:12:06'),
	(25, 13, '식사 약속', '2025-12-22 15:00:00', '2025-12-22 15:00:00', 5, '2025-12-23 02:33:50', '2025-12-23 02:33:50'),
	(26, 13, '회계감사', '2025-12-07 15:00:00', '2025-12-09 15:00:00', 1, '2025-12-23 05:04:43', '2025-12-23 05:04:43'),
	(28, 13, '연말회의 (2차)', '2025-12-08 15:00:00', '2025-12-10 15:00:00', 2, '2025-12-23 05:17:38', '2025-12-23 05:17:38'),
	(29, 13, '물건 준비 및 배치', '2025-12-23 15:00:00', '2025-12-23 15:00:00', 5, '2025-12-23 05:19:56', '2025-12-23 05:19:56'),
	(30, 13, '식사 약속', '2025-12-24 15:00:00', '2025-12-24 15:00:00', 5, '2025-12-23 05:20:14', '2025-12-23 05:20:14'),
	(31, 13, '18:30 ~ 팀 회식', '2025-12-29 15:00:00', '2025-12-29 15:00:00', 2, '2025-12-23 05:20:53', '2025-12-23 05:20:53'),
	(32, 13, '회의 자료 준비', '2025-12-05 15:00:00', '2025-12-05 15:00:00', 2, '2025-12-23 05:21:29', '2025-12-23 05:21:29'),
	(33, 13, '업체 방문', '2025-12-15 15:00:00', '2025-12-15 15:00:00', 1, '2025-12-23 05:22:18', '2025-12-23 05:22:18'),
	(66, 13, '새 일정12', '2025-11-25 15:00:00', '2025-11-27 15:00:00', 0, '2025-12-23 07:26:08', '2025-12-23 07:26:08'),
	(67, 13, '11월 결산', '2025-11-25 15:00:00', '2025-12-02 15:00:00', 0, '2025-12-23 07:29:08', '2025-12-23 07:29:08'),
	(68, 15, '김민수 일정', '2025-12-22 15:00:00', '2025-12-22 15:00:00', 0, '2025-12-23 13:30:42', '2025-12-23 13:30:42');
...
CREATE TABLE IF NOT EXISTS `tb_system_logs` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp() COMMENT '레코드 생성시각',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=52 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='시스템 로그';

-- 테이블 데이터 community.tb_system_logs:~36 rows (대략적) 내보내기
INSERT INTO `tb_system_logs` (`id`, `category`, `message`, `created_at`) VALUES
	...
	(46, 'post', '새 게시글: [자유 게시판] 1', '2025-12-12 08:09:31'),
	(47, 'post', '새 게시글: [자유 게시판] 새 게시물 테스트', '2025-12-15 10:05:58'),
	(50, 'project', '새 프로젝트: 새 프로젝트', '2025-12-15 11:09:59');

-- 뷰 community.v_category 구조 내보내기
...
CREATE TABLE `v_category` (
	...
	`post_count` DECIMAL(10,0) NOT NULL,
	`created_at` DATETIME NOT NULL,
	`updated_at` DATETIME NOT NULL
);

-- 뷰 community.v_chat_members 구조 내보내기
-- VIEW 종속성 오류를 극복하기 위해 임시 테이블을 생성합니다.
...
-- 뷰 community.v_chat_messages 구조 내보내기
-- VIEW 종속성 오류를 극복하기 위해 임시 테이블을 생성합니다.
CREATE TABLE `v_chat_messages` (
	`Id` INT(10) UNSIGNED NOT NULL COMMENT '레코드 ID',
	`Chat_Room_Id` INT(10) UNSIGNED NOT NULL COMMENT '채팅방 ID',
	`Employee_Id` INT(10) UNSIGNED NOT NULL COMMENT '직원 ID',
	`Employee_Name` VARCHAR(1) NOT NULL COMMENT '직원 이름' COLLATE 'utf8mb4_general_ci',
	`Message` VARCHAR(1) NOT NULL COMMENT '메시지 내용' COLLATE 'utf8mb4_general_ci',
	`Created_At` VARCHAR(1) NULL COLLATE 'utf8mb4_uca1400_ai_ci',
	`Deleted_At` VARCHAR(1) NULL COLLATE 'utf8mb4_uca1400_ai_ci'
);
...
CREATE TABLE `v_posts` (
	`id` INT(10) UNSIGNED NOT NULL COMMENT '레코드 ID',
	`post_category_id` INT(10) UNSIGNED NOT NULL,
	`post_category_name` VARCHAR(1) NULL COMMENT '카테고리 이름' COLLATE 'utf8mb4_general_ci',
	`parent_id` INT(10) UNSIGNED NULL COMMENT '상위 카테고리 id',
	`employee_id` INT(10) UNSIGNED NOT NULL COMMENT '작성자 직원 ID',
	`employee_name` VARCHAR(1) NULL COMMENT '직원 이름' COLLATE 'utf8mb4_general_ci',
	...
	`comment_count` INT(10) UNSIGNED NOT NULL COMMENT '댓글 수',
	`post_level` INT(10) UNSIGNED NULL COMMENT '게시물 수준 (0=시스템전체, 1=전체, 2=상위카테고리, 3=하위카테고리)',
...
-- 뷰 community.v_schedule 구조 내보내기
-- VIEW 종속성 오류를 극복하기 위해 임시 테이블을 생성합니다.
CREATE TABLE `v_schedule` (
	`id` INT(10) UNSIGNED NOT NULL COMMENT '레코드 id',
	`title` VARCHAR(1) NOT NULL COMMENT '일정 제목' COLLATE 'utf8mb4_general_ci',
	`start_at` TIMESTAMP NOT NULL COMMENT '일정 시작',
	`start_year` INT(5) NULL,
	`start_month` INT(3) NULL,
	`end_at` TIMESTAMP NOT NULL COMMENT '일정 종료',
	`end_year` INT(5) NULL,
	`end_month` INT(3) NULL,
	`color_code` INT(11) NOT NULL COMMENT '일정 색상',
...
-- 트리거 community.tr_post_comment_delete 구조 내보내기
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tr_post_comment_delete` AFTER DELETE ON `tb_post_comments` FOR EACH ROW BEGIN

	UPDATE tb_posts p 
	SET p.comments = IF(p.comments > 0, p.comments - 1, p.comments)
...
-- 트리거 community.tr_post_comment_insert 구조 내보내기
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tr_post_comment_insert` AFTER INSERT ON `tb_post_comments` FOR EACH ROW BEGIN

	UPDATE tb_posts p 
	SET 
		p.comment_count = (p.comment_count + 1), 
		p.comment_at = CURRENT_TIMESTAMP()
...
-- 트리거 community.tr_post_delete 구조 내보내기
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tr_post_delete` AFTER DELETE ON `tb_posts` FOR EACH ROW BEGIN

	UPDATE tb_post_category c
	SET c.post_count = IF(c.post_count > 0, c.post_count - 1, c.post_count)
	WHERE c.id = OLD.post_category_id;
	
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- 트리거 community.tr_post_insert 구조 내보내기
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `tr_post_insert` AFTER INSERT ON `tb_posts` FOR EACH ROW BEGIN
	SET @post_category = (SELECT IFNULL(c.name, 'None') FROM tb_post_category c  WHERE c.id = NEW.post_category_id LIMIT 1);
	SET @category = 'post';
	SET @message = CONCAT('새 게시글: ', '[', @post_category, '] ',  NEW.title);
	SET @created_at = NEW.created_at;
	INSERT INTO tb_system_logs (category, message, created_at) VALUES (@category, @message, @created_at);
	
	UPDATE tb_post_category c SET c.post_count = (c.post_count + 1) WHERE c.id = NEW.post_category_id;
END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_category` AS SELECT 
	0 AS `id`, 
	NULL AS `parent_id`, 
	'전체' AS `name`, 
	NULL AS `description`,
...
SELECT 
	c.id, 
	IF(c.parent_id IS NULL, 0, c.parent_id) AS `parent_id`, 
	c.`name`, 
	c.`description`, 
	c.post_count, 
	c.created_at, 
	c.updated_at 
FROM tb_post_category c 
WHERE 1 = 1 
;

-- 임시 테이블을 제거하고 최종 VIEW 구조를 생성
DROP TABLE IF EXISTS `v_chat_members`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_chat_members` AS SELECT 
	m.id,
	m.chat_room_id,
	m.employee_id,
	...
	m.created_at,
	m.deleted_at
FROM tb_chat_members m
	LEFT OUTER JOIN v_employees e ON e.id = m.employee_id 
;

-- 임시 테이블을 제거하고 최종 VIEW 구조를 생성
DROP TABLE IF EXISTS `v_chat_messages`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_chat_messages` AS SELECT 
	m.id AS 'Id',
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_schedule` AS SELECT 
	s.id,
	s.title,
	s.start_at,
	YEAR(s.start_at) AS `start_year`, 
	MONTH(s.start_at) AS `start_month`, 
	s.end_at,
	YEAR(s.end_at) AS `end_year`, 
	MONTH(s.end_at) AS `end_month`, 
	s.color_code,
	s.created_at,
	s.updated_at, 
	...
	e.rank_name
FROM tb_schedule s
	LEFT OUTER JOIN v_employees e ON e.id = s.employee_id 
;
